...
parser.add_argument('--skip_mouse_id', help="the mouse data to skip",
                    default='-1', type=str)
parser.add_argument('--task_name', help="the training task",
...
parser.add_argument('--patches_file', help=".t7 file containing patches",
                    default='')
parser.add_argument('--n_channel_convolution', default=128,
                    type=int)  # number of patches (or filters)
parser.add_argument('--spatialsize_convolution', default=4,
                    type=int)  # filter dimension
parser.add_argument('--padding_mode', default='constant',
...
parser.add_argument('--gaussian_patches', action='store_true',
                    help='patches sampled for gaussian RV')
parser.add_argument('--learn_patches', action='store_true',
                    help='learn the patches by SGD')
...
parser.add_argument('--dpp_subsample', action='store_true',
                    help='subsample patches with DPP')

# parameters of the classifier
parser.add_argument('--batch_norm', action='store_true',
                    help='add batchnorm before classifier')
parser.add_argument('--no_affine_batch_norm', action='store_true',
                    help='affine=False in batch norms')
...
parser.add_argument('--bottleneck_dim', default=0, type=int,
                    help='bottleneck dimension for the classifier')
parser.add_argument('--convolutional_classifier', type=int, default=0,
                    help='size of the convolution for convolutional classifier')
parser.add_argument('--bottleneck_spatialsize', type=int, default=1,
                    help='spatial size of the bottleneck')
parser.add_argument('--bottleneck_stride', type=int, default=1,
                    help='spatial size of the bottleneck')
parser.add_argument('--relu_after_bottleneck', action='store_true',
                    help='add relu after bottleneck ')
parser.add_argument('--bn_after_bottleneck', action='store_true',
                    help='add batch norm after bottleneck ')
parser.add_argument('--dropout', type=float, default=0.,
                    help='dropout after relu')
parser.add_argument('--feat_square', action='store_true',
                    help='add square features')
parser.add_argument('--resnet', action='store_true', help='resnet classifier')
parser.add_argument('--loss_type', default='cross_entropy_loss',
                    help='classifier/regeressor loss function')
...
# hardware parameters
parser.add_argument('--path_train', help="path to imagenet",
                    default=str(Path(os.getenv('PATH_TRAIN'))))
parser.add_argument('--path_test', help="path to imagenet",
                    default='/d1/dataset/imagenet32/out_data_val')
parser.add_argument('--path', help="path to imagenet",
                    default='/d1/dataset/2012')
parser.add_argument('--num_workers', type=int, default=2)
parser.add_argument('--multigpu', action='store_true')
parser.add_argument('--no_cudnn', action='store_true',
                    help='disable cuDNN to prevent cuDNN error (slower)')
parser.add_argument('--no_jit', action='store_true',
                    help='disable torch.jit optimization to prevent error (slower)')
parser.add_argument('--verbose', action='store_true',
                    help='print training epoch loss per batch')
...
parser.add_argument('--resume', default='',
                    help='filepath of checkpoint to load the model')
parser.add_argument('--summary_file',
                    default=str(Path(os.getenv('SUMMARY_FILE'))),
                    help='file to write summary')
...
whitening_eigvecs = whitening[
    'whitening_eigvecs']  # (16x16) for a patch_size of 4
...
if args.whitening_reg >= 0:
    ...
    inv_sqrt_eigvals = np.diag(
        1. / np.sqrt(whitening_eigvals + args.whitening_reg))
    whitening_op = whitening_eigvecs.dot(inv_sqrt_eigvals).astype(
        'float32')  # P @ Λ^(-1/2)
...
if hasattr(trainset, 'data'):
    ...
    patches = utils.select_patches_randomly(t,
                                            patch_size=spatialsize_convolution,
    ...
    patches /= 255.0
    print(
        f'patches randomly selected: {patches.shape}, mean {patches.mean()} std {patches.std()}')
    ...
    patches = utils.select_patches_from_loader(
        loader=trainloader_select_patches,
        batchsize=args.batchsize,
        patch_size=spatialsize_convolution,
        n_patches=n_channel_convolution,
...
WTW_patches = (patches).dot(whitening_op).dot(
    whitening_op.T)  # whitened patches  --> X @ P @ Λ^(-1) @ P^(T) = X @ ∑^(-1). # TODO: Don't we need X @ ∑^(-1/2)??
kernel_convolution = torch.from_numpy(WTW_patches.astype('float32')).view(
    orig_shape)
...
bias_convolution = torch.from_numpy(
    0.5 * W_patches_norm_square.astype('float32')).view(1, -1, 1, 1)
...
if args.gaussian_patches:
    ...
    kernel_convolution = torch.from_numpy(
        patches.dot(whitening_op.T).astype('float32')).view(orig_shape)
    patches_norm_square = np.linalg.norm(patches, axis=1) ** 2
    bias_convolution = torch.from_numpy(
        0.5 * patches_norm_square.astype('float32')).view(1, -1, 1, 1)
...
k_neighbors = args.kneighbors if args.kneighbors > 0 else int(
    n_channel_convolution * args.kneighbors_fraction)
...
out1, out2 = net(
    x)  # output after negative and positive convolution followed by pooling
if args.feat_square:
...
if args.spatialsize_convolution_2 > 0:
    ...
    patches_2 = utils.select_patches_from_loader(trainloader_select_patches,
                                                 args.batchsize,
                                                 args.spatialsize_convolution_2,
                                                 args.n_channel_convolution_2,
                                                 n_images_trainset,
                                                 n_patches_per_rowcol_2,
                                                 func=func,
                                                 seed=args.numpy_seed,
                                                 stride=1).numpy().astype(
        'float64')
    ...
    patches_mean_2, whitening_eigvecs_2, whitening_eigvals_2 = utils.compute_whitening_from_loader(
        trainloader_select_patches, patch_size=args.spatialsize_convolution_2,
        stride=1, func=func)
    print(
    ...
    if args.whitening_reg_2 >= 0:
        inv_sqrt_eigvals_2 = np.diag(
            1. / np.sqrt(whitening_eigvals_2 + args.whitening_reg_2))
        whitening_op_2 = whitening_eigvecs_2.dot(inv_sqrt_eigvals_2).astype(
            'float32')
    else:
        whitening_op_2 = np.eye(whitening_eigvals_2.size, dtype='float32')
    W_patches_2 = patches_2.dot(whitening_op_2)

    W_patches_2_norm_square = np.linalg.norm((patches_2).dot(whitening_op_2),
                                             axis=1) ** 2
    WTW_patches_2 = W_patches_2.dot(whitening_op_2.T)
    kernel_convolution_2 = torch.from_numpy(
        WTW_patches_2.astype('float32')).view(orig_shape_2)
    print(f'kernel convolution 2 shape: {kernel_convolution_2.shape}')

    bias_convolution_2 = torch.from_numpy(
        0.5 * W_patches_2_norm_square.astype('float32')).view(1, -1, 1, 1)
    ...
    net_2 = Net(kernel_convolution_2, bias_convolution_2,
                spatialsize_avg_pooling=1,
...
if args.resnet:
    ...
    classifier_blocks = [None, None, None, None, None,
                         None]  # batch_norm1, batch_norm2, batch_norm, classifier1, classifier2, classifier
else:
    classifier_blocks = utils.create_classifier_blocks(out1, out2, args, params,
                                                       n_classes)

print(f'Parameters shape {[param.shape for param in params]}')
print(
    f'N parameters : {sum([np.prod(list(param.shape)) for param in params]) / 1e6} millions')
...
if torch.cuda.is_available() and not args.no_jit:
    ...
    if args.batchsize_net > 0:
        trial = torch.rand(args.batchsize_net // n_gpus, args.input_channels,
                           spatial_size, spatial_size).to(device)
    else:
        trial = torch.rand(args.batchsize // n_gpus, args.input_channels,
                           spatial_size, spatial_size).to(device)
    if torch.cuda.is_available() and not args.learn_patches:
        trial = trial.half()
    ...
    with torch.jit.optimized_execution(True):
        net = torch.jit.trace_module(net, inputs, check_trace=False,
...
if args.normalize_net_outputs:
    ...
    if not os.path.exists(mean_std_file):
        mean1, mean2, std1, std2 = compute_channel_mean_and_std(trainloader,
                                                                net,
                                                                n_channel_convolution,
                                                                kernel_convolution,
                                                                bias_convolution,
                                                                n_epochs=1,
                                                                seed=0)
        np.savez(mean_std_file, mean1=mean1.cpu().numpy(),
                 mean2=mean2.cpu().numpy(), std1=std1.cpu().numpy(),
                 std2=std2.cpu().numpy())
        print(
            f'Net outputs mean and std computed and saved in file {mean_std_file}')
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        with torch.enable_grad() if args.learn_patches else torch.no_grad():
            if args.batchsize_net > 0:
                ...
                for i in range(
                    ...
                    outputs.append(net(inputs_batch))
                outputs1 = torch.cat([out[0] for out in outputs], dim=0)
                outputs2 = torch.cat([out[1] for out in outputs],
                                     dim=0)  # concatening along the batch dimension
            else:
                inputs = inputs.to(device)
                outputs1, outputs2 = net(inputs)

            ######## TODO: Add HRank here on outputs1, outputs2
            avg_batch_rank_per_label = utils.calculate_HRank(outputs1, outputs2,
                                                             targets)
            feature_maps_rank_per_batch.append(avg_batch_rank_per_label)
            ########

            if net_2 is not None:
                outputs1, outputs2 = net_2(
                    torch.cat([outputs1, outputs2], dim=1).float())

            if args.feat_square:
                outputs1 = torch.cat([outputs1, outputs1 ** 2],
                                     dim=1)  # concatening along the channel dimension
        ...
        if args.resnet:
            ...
            outputs, targets = utils.compute_classifier_outputs(outputs1,
                                                                outputs2,
                                                                targets, args,
                                                                batch_norm1,
                                                                batch_norm2,
        ...
        if args.verbose:
            print("training:", epoch, batch_idx, inputs.shape, outputs.shape,
                  targets.shape, loss)
    ...
    print(
        'Train, epoch: {}; Loss: {:.2f} | Acc: {:.1f} ; kneighbors_fraction {:.3f}'.format(
            epoch, train_loss / (batch_idx + 1), train_acc,
            args.kneighbors_fraction))

    loss_log = Path(f"logs/{args.task_name}")
    loss_log.mkdir(parents=True, exist_ok=True)

    dict_to_save = {'epoch': epoch, 'loss': train_loss / (batch_idx + 1),
                    'acc': train_acc}
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(loader):
            ...
            if args.loss_type == "mse":
                ...
                topk = (1,)
                cor_top1 = utils.correct_topk(outputs, targets,
                                              topk=topk)  # TODO: understand what this does
                ...
                # original code
                cor_top1, cor_top5 = utils.correct_topk(outputs, targets, topk=(
                    1, 3))  # TODO: understand what this does
...
hashname = hashlib.md5(
    str.encode(json.dumps(vars(args), sort_keys=True))).hexdigest()
if args.save_model:
...
if args.resume:
    ...
    if start_epoch not in learning_rates:
        ...
        if args.optimizer == 'Adam':
            optimizer = optim.Adam(params, lr=learning_rates[closest_i],
                                   weight_decay=args.weight_decay)
        elif args.optimizer == 'SGD':
            optimizer = optim.SGD(params, lr=learning_rates[closest_i],
                                  momentum=args.sgd_momentum,
                                  weight_decay=args.weight_decay)
        optimizer.load_state_dict(state['optimizer'])

    for block, name in zip(classifier_blocks,
                           ['bn1', 'bn2', 'bn', 'cl1', 'cl2', 'cl']):
...
def plot_roc_(y_pred, y_true, title="", mode=None, epoch=None):
    ...
    if mode == "auc_vs_dict_size":
        ...
        results_dir = Path(
            f'results/{args.task_name}/{args.spatialsize_convolution}')
        results_dir.mkdir(parents=True, exist_ok=True)

        dict_to_save["dictionary_size"] = args.n_channel_convolution

        with open(
                f'{results_dir}/auc_vs_dictionary_size_{args.nepochs}_{args.whitening_reg}.csv',
    ...
    if mode == "auc_vs_epochs":
        ...
        results_dir = Path(
            f'results/{args.task_name}/{args.spatialsize_convolution}/{args.n_channel_convolution}')
        results_dir.mkdir(parents=True, exist_ok=True)

        dict_to_save["epoch"] = epoch

        with open(f'{results_dir}/auc_vs_epochs_{args.whitening_reg}.csv', 'a',
                  newline='') as csvfile:
            # pickle.dump(scores_dict_to_save, f)
            fieldnames = list(dict_to_save.keys())
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    ...
    plt.title(title)
    plt.plot([0, 1], [0, 1], color='#C0C0C0',
             linewidth=0.6)  # diagonal grid line
    plt.show()
    plt.savefig(
        f"./figures/auc_roc_{args.task_name}_{args.spatialsize_convolution}_{args.n_channel_convolution}_{args.nepochs}.png")
...
for i in range(start_epoch, args.nepochs):
    if i in learning_rates:
        ...
        if args.optimizer == 'Adam':
            optimizer = optim.Adam(params, lr=learning_rates[i],
                                   weight_decay=args.weight_decay)
        elif args.optimizer == 'SGD':
            optimizer = optim.SGD(params, lr=learning_rates[i],
                                  momentum=args.sgd_momentum,
                                  weight_decay=args.weight_decay)
        else:
            raise NotImplementedError(
                'optimizer {} not implemented'.format(args.optimizer))
